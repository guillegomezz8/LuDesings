...
class CarritoPedidoTests(TestCase):
    ...
    def test_carrito_normaliza_talla_y_combina_al_cambiar_variante(self):
        StockTalla.objects.create(diseño=self.diseño, talla='S', stock=3)
        StockTalla.objects.create(diseño=self.diseño, talla='M', stock=3)
        ...
        self.client.post(reverse('actualizar_carrito', args=[self.diseño.id]), {
            'item_key': f'{self.diseño.id}:S',
            'talla': 'M',
            'cantidad': 2,
        })

        carrito = self.client.session['carrito']
        self.assertNotIn(f'{self.diseño.id}:S', carrito)
        self.assertEqual(carrito[f'{self.diseño.id}:M']['cantidad'], 3)

    @override_settings(STRIPE_SECRET_KEY='sk_test_fake', STRIPE_CURRENCY='eur')
    @patch('apps.tienda.views.pagos.stripe.checkout.Session.create')
    def test_checkout_crea_pedido_y_redirige_a_stripe(self, stripe_create):
        ...
        self.assertIsNotNone(pedido.fecha_expiracion_pago)
        ...
        self.assertNotIn('payment_method_types', stripe_kwargs)
        self.assertEqual(stripe_kwargs['locale'], 'es')
        self.assertEqual(stripe_kwargs['idempotency_key'], f'pedido-{pedido.pk}-checkout')
        self.assertIn('expires_at', stripe_kwargs)

    @override_settings(STRIPE_SECRET_KEY='sk_test_fake')
    @patch('apps.tienda.views.pagos.stripe.checkout.Session.retrieve')
    def test_stripe_success_no_descuenta_dos_veces_una_reserva(self, stripe_retrieve):
        self.diseño.stock = 1
        self.diseño.save(update_fields=['stock'])
        pedido = Pedido.objects.create(
            ...
            codigo_postal='28001',
            total=50,
            stripe_checkout_session_id='cs_test_reserva',
        ...
        stripe_retrieve.return_value = FakeStripeSession(
            id='cs_test_reserva',
            payment_status='paid',
            payment_intent='pi_test_reserva',
        )

        self.client.get(reverse('stripe_success'), {'session_id': 'cs_test_reserva'})
    ...
    def test_cancelar_checkout_libera_la_reserva(self):
        self.diseño.stock = 1
        self.diseño.save(update_fields=['stock'])
        pedido = Pedido.objects.create(
            nombre_cliente='Cliente Invitado',
        ...
        response = self.client.get(
            reverse('stripe_cancel', args=[pedido.pk]),
            {'token': str(pedido.access_token)},
        )
        ...
        self.assertEqual(pedido.estado_pago, 'fallido')
        self.assertFalse(pedido.stock_reservado)
        self.assertEqual(self.diseño.stock, 2)

    @override_settings(STRIPE_WEBHOOK_SECRET='whsec_test')
    @patch('apps.tienda.views.pagos.stripe.Webhook.construct_event')
    def test_webhook_expirado_libera_stock_de_la_talla(self, construct_event):
        stock_s = StockTalla.objects.create(diseño=self.diseño, talla='S', stock=1)
        pedido = Pedido.objects.create(
            nombre_cliente='Cliente Invitado',
            email='cliente@example.com',
            direccion='Calle Test 1',
        ...
        self.assertFalse(pedido.stock_reservado)
        self.assertEqual(pedido.estado_pago, 'fallido')
        self.assertEqual(stock_s.stock, 2)
